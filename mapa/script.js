document.addEventListener('DOMContentLoaded', function () {
  ...
  function createLocationCard(location) {
    ...
    locationCard.classList.add('card', 'mb-2', 'mx-3', 'listaCard');
    locationCard.innerHTML = `
      <div class="card-body">
        <div class="row g-0">
          <div class="col-md-2 d-flex justify-content-center align-items-center">
            <img src="/assets/img/0bebd305-5591-4afb-85d0-501540b3ffd0.svg" class="icon-gas" alt="...">
          </div>
          <div class="col-md-6">
            <h2 class="texto card-title">${location.name}</h5>
            <p class="card-text">Tipo: ${location.type}<br>Dimensión: ${location.dimension}</p>
          </div>
          <div class="col-md-4 mt-5">
            <button class="btn-facturar p-2 rounded" link="${location.url}">Facturar aquí</button>
          </div>
        </div>
      </div>
    `;
